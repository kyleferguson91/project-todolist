...
export function updateforhigh(value) {
...
(function sortweek() {
    ...
    o.defaultprojects.all.forEach((elem,ind,arr) => {
import('date-fns').then((date) => {
    // compare the date in the project to todays date
 if (date.parseISO(elem.duedate) <= date.addWeeks(parseISO(date.format(new Date(), 'yyyy-MM-dd')), 1))
    {
        // elements with today's date, use them to populate the today array in default
